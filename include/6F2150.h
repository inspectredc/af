#ifndef C_6F2150_H
#define C_6F2150_H

#include "ultra64.h"

void func_800CE4B0_jp(xyz_t*, f32*, s16*);
void func_800CE4F4_jp(xyz_t*, f32*, s16*);
void func_800CE554_jp(struct Actor*, xyz_t*);
void func_800CE694_jp(struct Actor*, xyz_t*);
// void func_800CE7E4_jp();
s16 func_800CE874_jp(struct Actor*);
void func_800CE8F0_jp(struct Actor*);
// void func_800CE9C4_jp();
s32 func_800CEB1C_jp();
// void func_800CEC30_jp();
s32 func_800CEC98_jp(struct Actor*, f32*, f32*, s16*, s16*, f32);
// void func_800CEE24_jp();
// void func_800CEED4_jp();
// void func_800CF05C_jp();
// void func_800CF16C_jp();

#endif
